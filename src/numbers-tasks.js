...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const aMagn = (x1 ** 2 + y1 ** 2) ** (1 / 2);
  const bMagn = (x2 ** 2 + y2 ** 2) ** (1 / 2);
  return Math.acos(dotProduct / (aMagn * bMagn));
}
...
function toNumber(value, def) {
  return +value ? Number(value) : Number(def);
}
...
function getFibonacciNumber(index) {
  ...
  if (index < 2) return fibArr[index];
